...
PlantsDataCenter 是一个**植物学参考数据集与结构化数据仓库**，主体记录在大连实际遇到、观察或整理过的
植物；少量为物种对比与学习而收录、但尚未在大连发现过的植物，用可选字段 `是否发现: 否` 标记。
维护者本地可保存原始 WPS/Excel 资料；仓库包含 `data/` 下的逐物种 YAML 真相源、Python 数据管线和
`unittest` 测试。这里的工作以数据整理、校验、导出和 AI/RAG 检索为主。
...
- `中文名` — 中文名称
- `是否发现` — 可选字段；只在该物种为对比学习而收录、但尚未在大连发现过时出现，当前取值为 `否`
- `俗名` — 俗名/别名（多个，逗号分隔）
...
- **`data/` 是真相源**。每个物种一个文件，路径为 `data/<拼音首字母>-<中文科名>/<物种>.yaml`（科目录名
  与对应 `knowledge/*.xlsx` 文件名去掉 `.xlsx` 后一致）。全量共 **58 科 165 物种**，其中大连已发现
  **154 物种**，对比学习收录但尚未发现 **11 物种**。每条记录含 **13 个必填字段**：
  `学名` `中文名` `俗名` `异名` `描述` `分类系统` `物种保护` `分类信息` `形态特征`
  `生态习性` `功用价值` `植物志` `元数据`。字段缺失时**补占位**（映射型区块为 `暂无数据`、
  列表型字段为 `无`），保证结构齐整。无法归类的段落会兜底进 `备注` 字段（导入时打印 `WARN`，
  属正常，不代表出错）。`是否发现` 是可选字段，有值时位于 `中文名` 后；未标记物种不写该字段，
  不补 `暂无数据`。
- **`DATA_STATS.md` 是根目录统计表**，记录当前科、属和物种数量，以及逐科、松科/柏科与木本/草本生活型的物种总数和大连发现/未发现拆分。有未发现物种时显示为“总数(大连发现/未发现)”，全部已发现时只显示总数。
  更新 README、AGENTS、CLAUDE 或任何数据规模相关文档时，必须同步核对并更新该文件。
...
- **脚本**（`scripts/`，均以 `python3 scripts/<名>.py` 运行，自带 `sys.path` 引导）。四个 CLI
  ...
  - `import_xlsx.py knowledge/*.xlsx` — 从 xlsx 解析并写出 `data/**/*.yaml`（幂等，重跑覆盖同名
    ...
    告警。
  - `validate.py` — 校验 `data/` 下全部记录：13 个必填字段齐全、可选 `是否发现` 取值、占位放行、
...
- **改动 xlsx 后重跑**：`python3 scripts/import_xlsx.py knowledge/<该文件>.xlsx`（或 `knowledge/*.xlsx`
  ...
  改名的物种会在 `data/` 留下旧文件，需手动删或先 `rm -rf data/<拼音首字母>-<科>` 再重导。
