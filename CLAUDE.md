...
仓库包含原始 WPS/Excel 资料、`data/` 下的逐物种 YAML 真相源、Python 数据管线和 `unittest`
测试；这里的工作以数据整理、校验、导出和 AI/RAG 检索为主。
...
- **`data/` 是真相源**。每个物种一个文件，路径为 `data/<拼音首字母>-<中文科名>/<物种>.yaml`（科目录名
  与对应 `knowledge/*.xlsx` 文件名去掉 `.xlsx` 后一致）。全量共 **46 科 122 物种**。每条记录含 **13 个必填字段**：
