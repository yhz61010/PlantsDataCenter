...
- **`data/` 是真相源**。每个物种一个文件，路径为 `data/<拼音首字母>-<中文科名>/<物种>.yaml`（科目录名
  与对应 `knowledge/*.xlsx` 文件名去掉 `.xlsx` 后一致）。全量共 **55 科 159 物种**，其中大连已发现
  **150 物种**，对比学习收录但尚未发现 **9 物种**。每条记录含 **13 个必填字段**：
  ...
  不补 `暂无数据`。
- **`DATA_STATS.md` 是根目录统计表**，记录当前科、属和物种数量、大连已发现数量，以及松科/柏科与木本/草本生活型统计。
