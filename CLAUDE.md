...
PlantsDataCenter 是一个**植物学参考数据集与结构化数据仓库**，记录在大连实际遇到、观察或整理过的
植物。仓库包含原始 WPS/Excel 资料、`data/` 下的逐物种 YAML 真相源、Python 数据管线和 `unittest`
...
- **`data/` 是真相源**。每个物种一个文件，路径为 `data/<科>/<物种>.yaml`（科名取自
  xlsx 文件名去掉拼音前缀的部分）。全量共 **44 科 158 物种**。每条记录含 **13 个固定字段**：
...
- **测试**：`python3 -m unittest discover -s tests`（当前 59 项，无 pytest）。
