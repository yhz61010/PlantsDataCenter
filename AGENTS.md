...
修改解析、导入、校验、导出或 YAML 序列化逻辑时，在 `tests/test_*.py` 中补充 `unittest`。测试应覆盖真实记录结构、占位归一、重复中文名、非法 YAML、学名格式、导出 JSON/Markdown/SQLite 等有业务价值的边界。交付前至少运行：
