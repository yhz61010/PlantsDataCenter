...
_NAME_PINYIN_TAIL = re.compile(r"[(（]([^)）]*)[)）]\s*$")


def _clean_name(s):
    # 个别源数据把拼音写进了中文名（如 "蜀葵 (shǔ kuí)"）；去掉尾部拼音括注。
    # 仅当括注内不含中文时才剥离，避免误删真正的中文别名注释。
    m = _NAME_PINYIN_TAIL.search(s)
    if m and not _CJK.search(m.group(1)):
        s = s[:m.start()]
    return s.strip()
